classdef my_deform < handle
    % change log: add support for function arap_subspace.
  ...
  methods(Access=public)
    ...
    function this = my_deform(varargin)
    % See also: deform
    ...
    function this = update_positions(this)
      ...
      case 'arap_subspace'
        if isempty(this.Tets)         
          if 1 % isempty(this.W)
            this.ba = farthest_point_sampling(this.V,[],this.F,b1',ceil(size(this.V,1)/1),-1);
            this.W = BilaplacianCoordinates(this.V,this.F,this.ba);
          end
          [Var,this.data] = arap_subspace( ...
            this.V,this.F,1:length(b1),bc1, ...
            'Bases',this.W,'Var0',this.V(this.ba,:),'MaxIter',this.max_iterations, ...
            'Energy',this.arap_energy, ...
            'Groups', this.G,'Tol',this.tol,'Data',this.data);
          this.new_V = this.W * Var;
        else
          if isempty(this.W)
            this.ba = farthest_point_sampling(this.V,this.Tets,[],b1',ceil(size(this.V,1)/1),-1);
            this.W = BilaplacianCoordinates(this.V,this.Tets,this.ba);
          end
          [Var,this.data] = this.W*arap_subspace( ...
            this.V,this.Tets,1:length(b1),bc1, ...
            'Bases',this.W,'Var0',this.V(this.ba,:),this.max_iterations, ...
            'Energy',this.arap_energy, ...
            'Groups', this.G,'Tol',this.tol,'Data',this.data);
          this.new_V = this.W * Var;
        end
