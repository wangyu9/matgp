function D = heat_divergence_distance(V,F,b,varargin)
...
if(full_kernel)
    ...
    HK = heat_kernel(EV,ED,t,[(1:n)'],'zero_eigen_skipped',zero_skipped);
    %EK = eigen_kernel(EV,ED,[(1:n)'],'zero_eigen_skipped',zero_skipped);
    % normalize
    %te = 1e-60;
    %HK(find(HK<te)) = te;
    %HK = bsxfun(@rdivide,HK,sum(HK,1));
end
%%
...
JS = @(p,q) max(0, p.*log(2*p./(q+p))+q.*log(2*q./(p+q)) ); % this kernel
...
p = HK(:,b);
w = diag(M);
D_sq = 2*sqrt(2*t)*full(sum( bsxfun(DIVER,(p)',HK) ,2));
