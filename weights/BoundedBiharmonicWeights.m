function W = BoundedBiharmonicWeights(varargin)
  ...
  if(facet_lap)
      ...
      if(size(F,2)==4)
        ...
        [Lf,facet] = facet_laplacian(V,F);
        % Mask for boundary *facets*
        isb = is_boundary_facet(facet,F);
        % Compute CR massmatrix for *facets*
        [Mass,MF] = crouzeix_raviart_massmatrix(V,F);
        assert(all(facet(:)==MF(:)));
        ...
        [Mass,mE] = crouzeix_raviart_massmatrix(V,F);
