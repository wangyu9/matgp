function varargout = simple_deform_1D(varargin)
  % SIMPLE_DEFORM Deform a mesh (V,F) using 
  % (1) linear blend skinning or 
  % (2) dual quaternion
  % (3) linear interpolate V=W*C;
  ...
  BE = [];
  % Be sure that control vertices are in 1D
  %assert(size(C,2)==1);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ii = 5;% this is used in the rest of the parser
  while(ii <= size(varargin,2))
    ...
    elseif(strcmp(varargin{ii},'InterpMode'))
      ...
      elseif( strcmp(varargin{ii},'DQLBS')) 
        interp_mode = 'DQLBS';
      elseif( strcmp(varargin{ii},'LI')) 
        interp_mode = 'LI';
      elseif( strcmp(varargin{ii},'visualizeSupport')) 
        interp_mode = 'visualizeSupport';
      elseif( strcmp(varargin{ii},'LBS_1D')) 
        interp_mode = 'LBS_1D';
      else
        error('Unknown InterpMode');
      end
    ...
    elseif(strcmp(varargin{ii},'PseudoEdges'))
      ...
      assert(ii<=size(varargin,2));
      PE = varargin{ii};
    elseif(strcmp(varargin{ii},'C0'))
      ii = ii + 1;
      assert(ii<=size(varargin,2));
      C0 = varargin{ii};      
    elseif(strcmp(varargin{ii},'R0'))
      ii = ii + 1;
      assert(ii<=size(varargin,2));
      R0 = varargin{ii};  
    else
...
%%%%%%Initalize 
  ...
  % by 1 list of angles
  if(size(R0,1)>0) % wangyu
    g_Deform(gid).R = R0;
  else
    g_Deform(gid).R = zeros(np,1);
  end
  if(size(C0,1)>0)
    % added by wangyu set the initial position of handles and apply skinning.
    g_Deform(gid).new_C = C0;
  else
  ...
  view_2D = true;
  if(view_2D)
      g_Deform(gid).tsh = plot(V(:,1),V(:,2));
        %trisurf(F,V(:,1),V(:,2),zeros(size(V,1),1),zeros(size(V,1),1), ...
        %'FaceColor','interp');
      % 2D view
      view(2);
      axis equal
      ...
      g_Deform(gid).tsh = trisurf(F,V(:,1),V(:,2),V(:,3),zeros(size(V,1),1), ...
        'FaceColor','interp');
      % 2D view
      view(3);        
  end
  ...
  if(show_weight_visualization)
    ...
    g_Deform(gid).wvsh = ...
        plot(V(:,1),V(:,2));
    %  trisurf(F,V(:,1),V(:,2),WVW(:,1),WVW(:,1));%, 'FaceColor','interp');%zeros(size(V,1),1)
    view(3);
    %axis equal
    %axis manual
    caxis([-1.3 1.3])
...
%   end  
  ...
  if(size(g_Deform(gid).new_C,1)==size(C,1))
    update_positions();
  end
  
    % keep track of window xmin, xmax, ymin, ymax
  win_min = min([g_Deform(gid).new_C(:,1:2); new_V(:,1:2)]);
  win_max = max([g_Deform(gid).new_C(:,1:2); new_V(:,1:2)]);
  t = max(win_max) - min(win_min);
  win_min = win_min - 0.1*t;
  win_max = win_max + 0.1*t;
  axis(reshape([win_min;win_max],1,4))
  ...
  function oncontrolsdown(src,ev)
    ...
    % keep track of mesh vertices at mouse down
    %down_V = get(g_Deform(gid).tsh,'Vertices');%wangyu 2014.9.18
    down_V = [get(g_Deform(gid).tsh,'XData')',get(g_Deform(gid).tsh,'YData')'];%
    %down_V = down_V(:,1:2);
    ...
    [found, temp_iP] = ismember(ci,P);
    [found_in_iP,~] = ismember(ci,iP);
    if(~found_in_iP)
        iP = [temp_iP];temp_iP;%[iP;temp_iP]
    end
    if(found)
      ...
      if(show_weight_visualization)
        ...
        index = find(temp==0); %Set the weights which is exact 0 to be a different color. 
        temp(index) = -100;
        
        %wangyu 2014.9.18
        %set(g_Deform(gid).wvsh,'Vertices',[V(:,1),V(:,2),sum(WVW(:,iP),2)]);
        %set(g_Deform(gid).wvsh,'CData',temp);%WVW(:,iP)
  ...
  function oncontrolsdrag(src,ev)
    ...
    if(strcmp('left',down_type))
      % move selected control points group by drag offset
      g_Deform(gid).new_C(iP,:) = ...
        bsxfun(@plus,g_Deform(gid).new_C(iP,:),drag_pos-last_drag_pos);
    else
      [found, iP] = ismember(ci,P);
      if(found)
         if(strcmp(interp_mode,'LI'))
            g_Deform(gid).new_C(iP,:) = ...
                rotate_around_center(g_Deform(gid).new_C(iP,:), 2*pi*(drag_pos(1)-last_drag_pos(1))/100 );  
         else
            g_Deform(gid).R(iP) = ...
    ...
    update_positions();
    % writeOBJ('temp.obj',new_V,F);
    % writeDMAT('H_bc.dmat',new_C);
    function [output_points] = rotate_around_center(input_points, angle)% added by wangyu
        rotate_center = mean(input_points,1);
        output_points(:,1) = (input_points(:,1)-rotate_center(1)).*cos(angle)-...
            (input_points(:,2)-rotate_center(2)).*sin(angle)+rotate_center(1);
        output_points(:,2) = (input_points(:,1)-rotate_center(1)).*sin(angle)+...
            (input_points(:,2)-rotate_center(2)).*cos(angle)+rotate_center(2);
    end
  end

function [TR] = skinning_transformations_1D(C,new_C,R)
    TR = zeros(size(C,1),2);
    TR(:,1) = tan(R+pi/4);
    TR(:,2) = new_C - tan(R+pi/4).*C; %(1-tan(R)).*C+new_C-C;
end

function [V_new] = lbs_1d(V,TR,W)
    P = [V,ones(size(V,1),1)];
    TV = V*TR(:,1)'+ones(size(V,1),1)*TR(:,2)';
    V_new = sum(TV.*W,2);
end
  ...
  function update_positions()
    ...
    if(strcmp(interp_mode,'LBS_1D'))
        g_Deform(gid).new_C(:,1) = C(:,1);
        TR = skinning_transformations_1D(C(:,2),g_Deform(gid).new_C(:,2),g_Deform(gid).R);
        new_V(:,2) = lbs_1d(V(:,1),TR,W);
    elseif(strcmp(interp_mode,'LBS'))
    % USING LINEAR BLEND SKINNING 
      ...
      %new_V = W*g_Deform(gid).new_C(P,:);
      % linear blend skinning
      [new_V] = lbs(V(:,1:2),TR,W);
      
    elseif(strcmp(interp_mode,'LI'))
      % get transformations stored at each point and bone handle
      TR = ...
        skinning_transformations(C,P,BE,g_Deform(gid).new_C,g_Deform(gid).R);
      if strcmp(auto_dof,'pseudoedges')
      ...
      % linear blend skinning
      %[new_V] = lbs(V(:,1:2),TR,W);
      new_V = W*g_Deform(gid).new_C;%wangyu
    ...
    elseif(strcmp(interp_mode,'visualizeSupport'))  
        ;% do nothing
    end

    % update mesh positions
    % wangyu 2014.9.18 set(g_Deform(gid).tsh,'Vertices',new_V(:,1:2));
    set(g_Deform(gid).tsh,'XData',new_V(:,1),'YData',new_V(:,2));
  ...
  function oncontrolsup(src,ev)
    ...
    %cur_V = get(g_Deform(gid).tsh,'Vertices');%wangyu 2014.9.18
    cur_V = [get(g_Deform(gid).tsh,'XData')',get(g_Deform(gid).tsh,'YData')'];
  ...
  function onkeypress(src,ev)
    ...
    elseif(strcmp(ev.Character,'u'))
      update_positions();
    elseif(strcmp(ev.Character,'t'))
      %g_Deform(gid).new_C = C;
      g_Deform(gid).R = g_Deform(gid).R + 0.1*ones(np,1);
      g_Deform(gid).new_C = rotate_all_points( g_Deform(gid).new_C, 0.1*ones(np,1) );
      update_positions();
    ...
    elseif(strcmp(ev.Character,'a'))
       if(strcmp(interp_mode,'LI'))
         g_Deform(gid).new_C(iP,:) = ...
          rotate_around_center(g_Deform(gid).new_C(iP,:), 2*pi*1/100 );  
       end
       update_positions();
    end
    oncontrolsup(src,ev);
      function [output_points] = rotate_all_points(input_points, angle)% added by wangyu
        rotate_center = mean(input_points,1);
        output_points(:,1) = (input_points(:,1)-rotate_center(1)).*cos(angle)-...
            (input_points(:,2)-rotate_center(2)).*sin(angle)+rotate_center(1);
        output_points(:,2) = (input_points(:,1)-rotate_center(1)).*sin(angle)+...
