function varargout = simple_deform_3D(varargin)
...
%   end
  ...
  if(show_weight_visualization)
    ...
    % plot the original mesh
    g_Deform(gid).wvsh = ...
      trisurf(F,V(:,1),V(:,2),V(:,3),WVW(:,1));%, 'FaceColor','interp');%zeros(size(V,1),1)
    %g_Deform(gid).wvsh = tetramesh(T,V,WVW(:,1)); 
    view(3);
    %axis equal
    %axis manual
    caxis([-1.3 1.3])
...
%         caxis([-1.3 1.3])
%         hold off;
        set(g_Deform(gid).wvsh,'Vertices',[new_V(:,1),new_V(:,2),new_V(:,3)]);
  ...
  function update_positions_3D()
    ...
    elseif(strcmp(interp_mode,'LI'))
      % get transformations stored at each point and bone handle
%       TR = ...
%         skinning_transformations(C,P,BE,[g_Deform(gid).new_C,C(:,3)],g_Deform(gid).R);%wangyu
%       if strcmp(auto_dof,'pseudoedges')
%         TR(:,:,1:np) = pseudoedge_dof( ...
%           C(P,:),PE,g_Deform(gid).new_C(P,:)-C(P,:), ...
%           axisangle2quat(repmat([0 0 1],np,1),g_Deform(gid).R));
%       end
      ...
      % linear blend skinning
      %[new_V] = lbs(V(:,1:2),TR,W);
      new_V = W*[g_Deform(gid).new_C(P,:),C(P,3)];%wangyu
