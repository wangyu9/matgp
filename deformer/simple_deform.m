  ...
  while(ii <= size(varargin,2))
    ...
    elseif(strcmp(varargin{ii},'InterpMode'))
      ...
      elseif( strcmp(varargin{ii},'visualizeSupport')) 
        ...
        error('InterpMode must be either LBS, DQLBS or LI');
      end
    ...
    elseif(strcmp(varargin{ii},'PseudoEdges'))
      ...
      assert(ii<=size(varargin,2));
      PE = varargin{ii};
    elseif(strcmp(varargin{ii},'InitHandlePos'))
      ii = ii + 1;
      assert(ii<=size(varargin,2));
      C0 = varargin{ii};      
    else
...
%%%%%%Initalize 
  ...
  view_2D = true;
  if(view_2D)
      g_Deform(gid).tsh = trisurf(F,V(:,1),V(:,2),zeros(size(V,1),1),zeros(size(V,1),1), ...
        'FaceColor','interp');
      % 2D view
      view(2);
      axis equal
...
%         subplot(1,number_of_subplots,1);
%         subplot(1,number_of_subplots,2);%current_subplot
%         hold on;
%         g_Deform(gid).wvsh = trisurf(F,V(:,1),V(:,2),WVW(:,iP),WVW(:,iP));
%         caxis([-1.3 1.3])
%         hold off;
  ...
  function update_positions()
    ...
    elseif(strcmp(interp_mode,'LI'))
      ...
      % linear blend skinning
      %[new_V] = lbs(V(:,1:2),TR,W);
      new_V = W*g_Deform(gid).new_C(P,:);%wangyu
