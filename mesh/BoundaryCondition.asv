function [b,bc] = boundary_conditions(V,F,C,P,E,CE,SCI,tSC,SVI,tSV)
  % BOUNDARY_CONDITIONS
  ...
  % SCI, all control vertices indices into the mesh
  % tSC, t value of all control vertices
  % SVI, all vertices's indicers in the mesh
  % tSV, t value of all other vertice in the spline
  % compute boundary conditions due to cage edges
  if(~isempty(SCI))

    CM = cubic_spline(tSC);
    [~,~,M] = cubic_spline_eval(tSC,tSV,CM,V(SCI,:));  
